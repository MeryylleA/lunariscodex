...
class MemmapCodeDataset(Dataset):
    def __init__(
        ...
        logger.info(
            f"Loading dataset from {memmap_file} with {num_sequences} sequences and max_length {max_length}"
        )
        dtype = np.int16 if dtype_str == "int16" else np.int32
        try:
            self.data = np.memmap(
                memmap_file, dtype=dtype, mode="r", shape=(num_sequences, max_length)
            )
            ...
            logger.error(
                f"Error loading memmap (check shape/dtype?): {memmap_file} - {e}"
            )
            raise
        ...
        logger.info(
            f"Dataset loaded. Pad ID for attention mask: {self.tokenizer_pad_id}"
        )
    ...
    def __getitem__(self, idx):
        ...
        attention_mask = (
            input_ids != self.tokenizer_pad_id
        ).long()
...
def compute_metrics(logits, targets, attention_mask):
    ...
    targets_shifted = targets[..., 1:].contiguous()   # (B, L-1)
    attention_mask_shifted = attention_mask[..., 1:].contiguous()  # (B, L-1)

    # Flatten logits and targets
    logits_flat = logits_shifted.view(-1, logits_shifted.size(-1))  # (B*(L-1), V)
    targets_flat = targets_shifted.view(-1)  # (B*(L-1),)

    # Apply attention mask to ignore padded tokens
    active_mask = attention_mask_shifted.view(-1).bool()  # (B*(L-1),)
...
def save_checkpoint(
    model, optimizer, epoch, step, current_loss, args, is_best=False, scheduler=None
):
...
def load_checkpoint(model, optimizer, args, device, scheduler=None):
    ...
    if (
        not checkpoint_to_load and args.checkpoint_dir
    ):
    ...
    if checkpoint_to_load and os.path.isfile(checkpoint_to_load):
        ...
        try:
            ...
            current_model_is_compiled = hasattr(model, "_orig_mod") and isinstance(
                model._orig_mod, nn.Module
            )
            checkpoint_was_from_compiled = any(
                k.startswith("_orig_mod.") for k in model_state_dict.keys()
            )
            ...
            missing_keys, unexpected_keys = (
                target_model_for_load_state_dict.load_state_dict(
                    model_state_dict, strict=False
                )
            )

            if missing_keys:
                logger.warning(f"Missing keys in model_state_dict: {missing_keys}")
            if unexpected_keys:
                logger.warning(
                    f"Unexpected keys in model_state_dict: {unexpected_keys}"
                )
            ...
            if (
                ...
                and (current_training_uses_lora == checkpoint_was_lora_training)
            ):
                try:
            ...
            logger.info(
                f"Checkpoint loaded. Resuming from epoch {start_epoch + 1}, global step {start_step}."
            )
        except Exception as e:
            logger.error(
                f"Failed to load checkpoint: {e}. Starting from scratch.", exc_info=True
            )
            start_epoch, start_step, min_val_loss = 0, 0, float("inf")
    else:
...
def train_model_loop(model, train_dataloader, val_dataloader, tokenizer, args):
    ...
    if not optimizer_params:
        logger.error(
            "No trainable parameters found for the optimizer! Check LoRA setup or model's requires_grad flags."
        )
        return model
    ...
    scaler = amp.GradScaler(
        enabled=(args.mixed_precision_dtype is not None and device.type == "cuda")
    )

    start_epoch, global_step, best_val_loss = load_checkpoint(
        model, optimizer, args, device, scheduler
    )

    if args.use_torch_compile and hasattr(torch, "compile"):
        logger.info(
            f"Compiling model with torch.compile (mode: {args.torch_compile_mode})..."
        )
        try:
            model_to_compile = (
                model._orig_mod
                if hasattr(model, "_orig_mod")
                and isinstance(model._orig_mod, nn.Module)
                else model
            )
            compiled_part = torch.compile(
                model_to_compile, mode=args.torch_compile_mode
            )
            if hasattr(model, "_orig_mod") and isinstance(model._orig_mod, nn.Module):
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        train_iterator = tqdm(
            train_dataloader, desc=f"Epoch {epoch+1}/{args.num_epochs} [Training]"
        )
        ...
        for batch_idx, batch in enumerate(train_iterator):
            ...
            if global_step == 0 and batch_idx == 0:
                logger.info(
                    f"First batch - Epoch {epoch+1}, Shape: {input_ids.shape}, Pad ID (for attention mask): {tokenizer.pad_token_id}"
                )
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                logger.error(
                    f"Invalid loss (NaN or Inf) at epoch {epoch+1}, batch {batch_idx}, global_step {global_step}! Stopping training."
                )
                return model
            ...
            if args.grad_clip_norm > 0:
                ...
                torch.nn.utils.clip_grad_norm_(
                    optimizer_params, max_norm=args.grad_clip_norm
                )
            ...
            epoch_perplexity += (
                current_ppl_item
                if not (torch.isinf(perplexity) or torch.isnan(perplexity))
                else (epoch_perplexity / (batch_idx + 1) if batch_idx > 0 else 0.0)
            )
            ...
            if global_step % args.log_interval == 0:
                logger.info(
                    f"E{epoch+1} S{global_step} B{batch_idx+1}/{len(train_dataloader)} | Loss: {loss.item():.4f}, PPL: {current_ppl_item:.2f}, Acc: {top1_acc.item():.3f}, LR: {optimizer.param_groups[0]['lr']:.2e}"
                )
            ...
            if args.save_strategy == "steps" and global_step % args.save_steps == 0:
                ...
                if (
                    ...
                    val_loss_for_save, _, _ = evaluate_model(
                        model, val_dataloader, tokenizer, args, device
                    )
                ...
                save_checkpoint(
                    ...
                    val_loss_for_save,
                    args,
                    is_best=is_best_for_save,
                    scheduler=scheduler,
                )

        avg_epoch_loss = (
            epoch_loss / len(train_dataloader) if len(train_dataloader) > 0 else 0
        )
        avg_epoch_perplexity = (
            epoch_perplexity / len(train_dataloader)
            if len(train_dataloader) > 0
            else float("inf")
        )
        avg_epoch_top1_acc = (
            epoch_top1_acc / len(train_dataloader) if len(train_dataloader) > 0 else 0
        )
        logger.info(
            f"End of Epoch {epoch+1}/{args.num_epochs} - Training | Avg Loss: {avg_epoch_loss:.4f}, Avg PPL: {avg_epoch_perplexity:.2f}, Avg Acc: {avg_epoch_top1_acc:.3f}"
        )
        ...
        if val_dataloader:
            ...
            logger.info(
                f"End of Epoch {epoch+1} - Validation | Loss: {val_loss:.4f}, PPL: {val_perplexity:.2f}, Acc: {val_accuracy:.3f}"
            )
            ...
            if is_current_epoch_best:
                ...
                logger.info(
                    f"New best validation loss: {best_val_loss:.4f} (at epoch {epoch+1})"
                )
...
def evaluate_model(model, dataloader, tokenizer, args, device):
    ...
    with torch.no_grad():
        for batch in eval_iterator:
            ...
            if torch.isnan(loss) or torch.isinf(loss):
                logger.warning(
                    "Invalid loss (NaN/Inf) during evaluation. Skipping batch."
                )
                continue
            ...
            total_perplexity += (
                current_ppl_item
                if not (torch.isinf(perplexity) or torch.isnan(perplexity))
                else (total_perplexity / (len(eval_iterator) or 1))
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the training .memmap dataset.",
    )
    parser.add_argument(
        "--num_sequences_train",
        type=int,
        required=True,
        help="Number of sequences in the training dataset.",
    )
    parser.add_argument(
        "--memmap_file_val",
        type=str,
        default=None,
        help="Path to the validation .memmap dataset (optional).",
    )
    parser.add_argument(
        "--num_sequences_val",
        type=int,
        default=0,
        help="Number of sequences in the validation dataset.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=1024,
        help="Max length used for dataset creation (for memmap shape).",
    )
    parser.add_argument(
        "--dataset_dtype",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Vocabulary size (if None, inferred from tokenizer).",
    )
    parser.add_argument(
        "--d_model", type=int, default=768, help="Model hidden dimension."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=1024,
        help="Model's internal max sequence length (for ALiBi).",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="LoRA rank (0 or negative to disable LoRA and train full model).",
    )
    ...
    parser.add_argument(
        "--adam_fused",
        action="store_true",
        help="Use NVIDIA's fused AdamW (CUDA only).",
    )
    parser.add_argument(
        "--grad_clip_norm",
        type=float,
    ...
    parser.add_argument(
        ...
        default=None,
        choices=["fp16", "bf16"],
        help="Enable mixed precision (fp16 or bf16). CUDA only. Default: fp32.",
    )
    parser.add_argument(
        "--device",
        type=str,
    ...
    parser.add_argument(
        "--pin_memory",
        action="store_true",
        help="Use pin_memory in DataLoader (CUDA only).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--log_interval", type=int, default=100, help="Log metrics every X steps."
    )
    parser.add_argument(
        "--validation_interval_steps",
        type=int,
        default=0,
        help="Run validation every X steps (0 for end-of-epoch only).",
    )
    ...
    parser.add_argument(
        "--allow_tf32",
        action="store_true",
        help="Allow TF32 on Ampere+ GPUs for matmuls.",
    )
    parser.add_argument(
        "--cudnn_benchmark",
        action="store_true",
        help="Enable cudnn.benchmark (if input sizes are fixed).",
    )
    ...
    if tokenizer.pad_token_id is None:
        pad_token_to_add = (
            ...
            else tokenizer.bos_token_id if tokenizer.bos_token_id is not None else None
        )
        if pad_token_to_add is not None:
            ...
            logger.warning(
                f"Tokenizer had no pad/eos/bos. Added new pad_token='<|PAD|>' (ID: {tokenizer.pad_token_id}). Vocab size is now: {len(tokenizer)}. Ensure model's vocab_size is updated if not inferred."
            )

    train_dataset = MemmapCodeDataset(
        args.memmap_file_train,
        args.num_sequences_train,
        args.dataset_max_length,
    ...
    if args.memmap_file_val and args.num_sequences_val > 0:
        val_dataset = MemmapCodeDataset(
            args.memmap_file_val,
            args.num_sequences_val,
            args.dataset_max_length,
            tokenizer.pad_token_id,
            args.dataset_dtype,
        )
        ...
        logger.info(
            f"Validation dataset loaded from {args.memmap_file_val} ({args.num_sequences_val} sequences)."
        )
    else:
        logger.info("No validation dataset provided or num_sequences_val is 0.")

    effective_vocab_size = (
        args.vocab_size if args.vocab_size is not None else len(tokenizer)
    )
    if args.vocab_size is not None and args.vocab_size != len(tokenizer):
        logger.warning(
            f"Provided --vocab_size ({args.vocab_size}) differs from tokenizer's actual vocab size ({len(tokenizer)}). Using --vocab_size."
        )
    ...
    if args.use_lora:
        ...
        logger.info(
            f"Total model parameters: {all_params_val:,}. Trainable (LoRA) parameters: {trainable_params_val:,} ({trainable_params_val/all_params_val*100:.2f}%)"
        )
    else:
        logger.info(
            "Setting up for full model training (LoRA rank is 0 or not specified)."
        )
        all_params_val = 0
        trainable_params_val = 0
        for param in model.parameters():
            ...
            if param.requires_grad:
                trainable_params_val += param.numel()
        logger.info(
            f"Total model parameters: {all_params_val:,}. Trainable parameters: {trainable_params_val:,} ({trainable_params_val/all_params_val*100:.2f}%)"
        )

    trained_model = train_model_loop(
        model, train_dataloader, val_dataloader, tokenizer, args
    )
