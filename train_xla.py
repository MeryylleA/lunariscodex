"""
Main Training Script for the LunarisCodex Language Model - TPU/XLA Version
...
This version has been refactored to run on Google Cloud TPUs using PyTorch/XLA.

Key Features:
- **TPU Distributed Training:** Utilizes PyTorch/XLA's multiprocessing to train the model
  across multiple TPU cores in a pod. XLA handles gradient synchronization automatically.
- **Optimized Data Loading:** Implements a custom `ShardDataset` that uses memory-mapping
  ...
  as needed.
- **Mixed-Precision Training:** Leverages `torch.amp.autocast` with `bfloat16` precision,
  which TPUs excel at, significantly speeding up training and reducing memory consumption.
- **XLA JIT Compilation:** The XLA backend itself is a JIT compiler, so explicit torch.compile
...
class TrainConfig:
    ...
    batch_size: int = 16 # Per-TPU core batch size
    ...
    device: str = "xla" # Device to train on (changed from 'cuda' to 'xla' for TPU)
    compile_model: bool = False # XLA is itself a JIT compiler, so torch.compile is not needed
...
# --- Worker Function for XLA Multiprocessing ---
def _mp_fn(index, config_path: str):
    """
    Worker function that will be spawned by xmp.spawn for each TPU core.
    This function contains the main training logic.
    
    Args:
        index: The index of the current process (TPU core)
        config_path: Path to the configuration file
    """
    # --- Setup ---
    config = TrainConfig.from_yaml(config_path)
    
    # XLA-specific: Get the device for this TPU core
    device = xm.xla_device()
    
    # XLA-specific: Check if this is the master process
    ...
    if is_master_process:
        ...
        print("-" * 50)
        print(" " * 15 + "LUNARIS CODEX TRAINING (TPU)")
        print("-" * 50)
        print(f"Model: {config.model}")
        print(f"Data: {config.data_dir}")
        print(f"Batch size: {config.batch_size} per TPU core")
    ...
    # XLA-specific: Create DistributedSampler with XLA world size and ordinal
    ...
    # XLA is itself a JIT compiler, so we don't need torch.compile
    # The model is used directly without DDP wrapper

    # --- Optimizer ---
    # Use the model's configure_optimizers method
    optimizer = model.configure_optimizers(
        weight_decay=config.weight_decay,
        learning_rate=config.learning_rate,
        betas=(config.beta1, config.beta2),
        device_type='xla'  # Changed from original device_type
    )
    ...
    # --- Training Loop ---
    # Set the sampler's epoch to ensure proper shuffling when resuming
    ...
    while current_step < config.max_steps:
        ...
        # XLA handles gradient synchronization automatically, so we don't need DDP's no_sync
        ...
        # XLA-specific: Use xm.optimizer_step instead of optimizer.step()
        # This triggers XLA graph execution, gradient all-reduce, and weight update
        ...
        if is_master_process:
            ...
            if current_step > 0 and current_step % config.save_interval == 0:
                ...
                # XLA-specific: Use xm.save instead of torch.save
                # This correctly handles tensor transfer from TPU to CPU
...
if __name__ == '__main__':
    # --- Script Entry Point ---
    # This is refactored to use XLA multiprocessing
    import argparse
    parser = argparse.ArgumentParser(description="Train a LunarisCodex model on TPU.")
    parser.add_argument("config", type=str, help="Path to the config.yaml file.")
    parser.add_argument("--nprocs", type=int, default=None, help="Number of TPU cores to use (defaults to all available)")
    args = parser.parse_args()
    
    # XLA-specific: Use xmp.spawn to launch the training across TPU cores
    # If nprocs is not specified, it will use all available TPU cores
    xmp.spawn(_mp_fn, args=(args.config,), nprocs=args.nprocs)
