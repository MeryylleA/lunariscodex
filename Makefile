...
EXE_SUFFIX :=
BPE_PROCESSOR_LINK_LIBS := -lstdc++fs # Padrão para Linux/MinGW

ifeq ($(OS_TYPE),Windows_NT)
    EXE_SUFFIX := .exe
else ifeq ($(OS_TYPE),MINGW32_NT-$(shell uname -m | cut -d'-' -f1))
	OS_TYPE := Windows_NT
    EXE_SUFFIX := .exe
    ...
    # No macOS (Darwin), -lstdc++fs não é necessário e pode causar erro com Clang.
    # <filesystem> é geralmente parte de libc++ com C++17.
    BPE_PROCESSOR_LINK_LIBS :=
endif
...
# BPE_PROCESSOR_LIBS foi movido para BPE_PROCESSOR_LINK_LIBS para condicional


$(info Makefile - OS_TYPE (final): $(OS_TYPE))
$(info Makefile - EXE_SUFFIX: $(EXE_SUFFIX))
$(info Makefile - BPE_PROCESSOR_LINK_LIBS: $(BPE_PROCESSOR_LINK_LIBS))
...
$(BPE_PROCESSOR_TARGET): $(BPE_PROCESSOR_SRC) $(JSON_HEADER) Makefile
	...
	@echo "Command: $(CXX) $(CURRENT_CXXFLAGS) $< -o $@ $(BPE_PROCESSOR_LINK_LIBS)"
	$(CXX) $(CURRENT_CXXFLAGS) $< -o $@ $(BPE_PROCESSOR_LINK_LIBS)
